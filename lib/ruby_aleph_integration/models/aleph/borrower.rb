module Aleph
  class Borrower < Base
    ...
    def loans
      #retrieve_loans if @loans.nil?
      @loans
    end

    def holds
      #retrieve_holds if @holds.nil?
      @holds
    end

    def cash
      #retreive_cash if @cash.nil?
      @cash
    end

    def photo
      nil
      #retreive_photo if @z38s.nil?
      #@z38s
    end
    ...
    def retrieve_loan
      @z36s = parse_objects(
        @config.aleph_loan_class,
        @@connection.patron_request(
          @aleph_pid,
    ...
    def retrieve_photo
      @z38s = parse_objects(
        @config.aleph_photo_class,
        @@connection.patron_request(
          @aleph_pid,
          'circulationActions/requests/photocopies'
        )
      )
    end
    ...
    def retrieve_holds
      @z37 = parse_objects(
        @@connection.patron_request(
          @aleph_pid, 
          'circulationActions/requests/holds'
        )
      )
    end
